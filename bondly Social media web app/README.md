...
|       +-- src/main/resources/
|           +-- application.yaml          # Local runtime config, should stay private
...
```bash
cd LishareFrontend/frontend/ui
npm install
npm run dev
```
...
Important: if `application.yaml` has already been tracked by Git before adding it to `.gitignore`, Git will still show changes for that file. `.gitignore` only protects new untracked files.

To stop tracking the real config while keeping the file on your computer:

```bash
git rm --cached Lisharebackend/Lishare/src/main/resources/application.yaml
git commit -m "Stop tracking local application config"
```

For a temporary local-only solution, this can hide local changes:

```bash
git update-index --skip-worktree Lisharebackend/Lishare/src/main/resources/application.yaml
```

That second command is only a convenience for local development. It is not a real security fix if secrets were already committed. If private credentials were pushed before, rotate the database password, mail app password, and JWT secret.

## Runtime Files

Uploaded images are stored in `uploads/` and served through `/uploads/**`. These files are runtime data, so they should normally stay out of Git unless I intentionally add demo assets.
